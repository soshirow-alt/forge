    ...
    return (
      <li className="rounded-xl border border-zinc-800/80 bg-zinc-950/30 px-4 py-4 sm:px-5 sm:py-5">
        ...
        <p className="mt-2 text-sm text-zinc-400">
          自由記述の声 <span className="font-semibold text-zinc-200">{totalResponses}</span> 件
        </p>
  ...
  if (!canShowPublicVoiceTrend(totalResponses)) {
    return (
      <li className="rounded-xl border border-zinc-800/80 bg-zinc-950/30 px-4 py-4 sm:px-5 sm:py-5">
        ...
        <p className="mt-2 text-xs leading-relaxed text-zinc-600">
          まだ声が集まりはじめたところです。もう少し回答が集まると、傾向が表示されます。
        </p>
      </li>
    );
  }
...
export function EveryonesVoiceSection({
  ...
  const header = (
    <>
      ...
      <p className={`${isTab ? "mt-2" : "mt-1"} text-xs leading-relaxed text-zinc-600`}>
        {isTab ? "プレイヤーの声の傾向" : `${versionLabel} への回答の集計です`}。
        個別の回答内容は公開されません。開発者には改善のヒントとして届きます。
      </p>
    </>
  );
    ...
    return (
      ...
      >
        ...
        <div className={`${isTab ? "mt-6" : "mt-3"} rounded-xl border border-dashed border-zinc-800 bg-zinc-950/20 px-4 py-8 text-center`}>
          <p className="text-sm text-zinc-400">まだ声が集まりはじめたところです</p>
          <p className="mt-2 text-xs leading-relaxed text-zinc-600">
            プレイ後、開発者の質問に答えると、ここに声の傾向が表示されます。
          </p>
          {onSendVoice ? (
            <button
              type="button"
  ...
  return (
    ...
    >
      {header}
      {isTab && totalResponseCount > 0 && totalResponseCount < 3 ? (
        <p className="mt-3 text-xs text-zinc-600">
          まだ声が集まりはじめたところです。もう少し回答が集まると、傾向が見えやすくなります。
        </p>
      ) : null}
