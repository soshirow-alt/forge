-- STAGING ONLY seed — Player IA phase 1 demo data
-- Target ref: vuqpwvjvgyxffmvpfrxo
-- DO NOT run on Production (bpnisgzxuwdxelhnduuf).
--
-- NOT a supabase/migrations/* file — never auto-apply with schema migrations.
-- Prerequisite schema: 076–081 applied on Staging.
--
-- Identifiers:
--   tag: forge-ia-seed-v1
--   title prefix: [IA Seed]
--   project UUIDs: eeeeeeee-eeee-4eee-8eee-000000000001 .. 0005
--   usage UUIDs:  ffffffff-ffff-4fff-8fff-000000000001 .. 0002
--   announcement UUIDs: aaaaaaaa-aaaa-4aaa-8aaa-000000000001 .. 0003
--   guest FB UUID: bbbbbbbb-bbbb-4bbb-8bbb-000000000001
--   submitter_key: bbbbbbbb-bbbb-4bbb-8bbb-000000000099
--   owner (existing Staging hero user): dddddddd-dddd-4ddd-8ddd-000000000002
--
-- Re-run safe: INSERT ... ON CONFLICT upserts seed rows only.
-- Does NOT delete/alter Smoke A/B or unrelated hero-carousel titles.
-- Cleanup: scripts/staging-only/player-ia-staging-seed-cleanup.sql

BEGIN;

-- Abort unless known Staging Smoke A project exists (wrong DB / Production guard).
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
    ...
    RAISE EXCEPTION
      'ABORT player-ia-staging-seed: Staging Smoke A missing — refuse to run (wrong project / Production?)';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION
      'ABORT player-ia-staging-seed: Staging hero-carousel project missing — refuse to run';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION
      'ABORT player-ia-staging-seed: projects.category missing — apply 076 first';
  END IF;
END $$;

-- Touch shared Staging owner profile for activity-tag / search checks.
-- Cleanup does NOT fully revert this shared row (see cleanup SQL comments).
