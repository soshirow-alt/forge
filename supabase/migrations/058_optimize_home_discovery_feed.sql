-- 058: get_home_discovery_feed performance (semantics unchanged)
--
-- Goal: cut Production RPC wall time (~2–4s with ~9 public projects) without
-- changing newest / updated / trending selection rules or card fields.
--
-- Changes (implementation only):
-- 1) Precompute project_id_text once on public_projects (avoid repeated uuid::text).
-- 2) Restrict 7d engagement aggregates to public_projects via join (no global-then-filter).
-- 3) Inline card stats for candidates only — feedback_participant_count + watch_count —
--    instead of calling get_public_project_stats (which also scanned witness grants +
--    latest_devlog unused by this feed).
--
-- Staging-first. Apply via Dashboard SQL. No data backfill. No mass indexes.
-- Prerequisite: 055 applied.
...
AS $$
  ...
  updated_agg AS (
    ...
    INNER JOIN meaningful_events me
      ON me.project_id_text = pp.project_id_text
  ...
  fb_7d AS (
    ...
    INNER JOIN (
      ...
      ON combined.project_id_text = pp.project_id_text
    GROUP BY pp.project_id
  ),
  watch_7d AS (
    SELECT
    ...
    FROM public_projects pp
    INNER JOIN public.project_watches w
      ON w.project_id = pp.project_id_text
    CROSS JOIN params prm
  ...
  play_7d AS (
    ...
    FROM public_projects pp
    INNER JOIN public.project_play_sessions s
      ON s.project_id = pp.project_id_text
    CROSS JOIN params prm
  ...
  trending_base AS (
    ...
    FROM public_projects pp
    LEFT JOIN fb_7d f ON f.project_id = pp.project_id
    LEFT JOIN watch_7d w ON w.project_id = pp.project_id
    LEFT JOIN play_7d pl ON pl.project_id = pp.project_id
  ...
  candidate_ids AS (
    SELECT DISTINCT ranked.project_id AS project_id
    FROM (
  ...
  -- Feed only needs participant + watch counts (not witness / latest_devlog).
  feedback_participants AS (
    SELECT
      c.project_id AS project_id,
      COUNT(DISTINCT combined.participant_key)::bigint AS feedback_participant_count
    FROM candidate_ids c
    INNER JOIN public_projects pp ON pp.project_id = c.project_id
    INNER JOIN (
      SELECT
        vr.project_id AS project_id_text,
        vr.user_id::text AS participant_key
      FROM public.project_voice_responses vr
      WHERE vr.moderation_status = 'visible'
        AND vr.user_id IS NOT NULL
      UNION
      SELECT
        fb.project_id AS project_id_text,
        fb.user_id::text AS participant_key
      FROM public.project_feedback fb
      WHERE fb.moderation_status = 'visible'
        AND fb.user_id IS NOT NULL
    ) combined
  ...
  stats AS (
    ...
    FROM candidate_ids c
    LEFT JOIN feedback_participants fp ON fp.project_id = c.project_id
    LEFT JOIN watch_counts wc ON wc.project_id = c.project_id
  ),
  newest_rows AS (
    SELECT
...
COMMENT ON FUNCTION public.get_home_discovery_feed() IS
  'Home discovery feed (newest/updated/trending). 058: same selection as 055; '
  'faster joins via project_id_text; 7d aggs scoped to public projects; '
  'inline candidate stats (no unused witness/devlog from get_public_project_stats).';
