-- 059: get_home_discovery_feed — http(s) thumbnail_url only on the wire
--
-- Root cause of 053/054 failures:
--   plpgsql + RETURNS TABLE(...) makes every output name a function variable.
--   CTE/SQL columns with the same names then raise 42702
--   (rank, then project_id, and any other RETURNS TABLE name next).
--
-- Definitive fix:
--   Rewrite as LANGUAGE sql. No plpgsql variables exist, so RETURNS TABLE
--   names cannot clash with SQL columns. Also qualify every CTE/JOIN column
--   and map the final SELECT with explicit AS aliases.
--
-- Fresh chain 050→055 and Staging (054 applied) + this file alone both end
-- on this same function body.
-- Hotfix after confirmed multi-MB data URL payloads. Selection unchanged from 055. 058 remains unapplied.
...
COMMENT ON FUNCTION public.get_home_discovery_feed() IS
  'Home discovery feed. 059: thumbnail_url is http(s) only (non-http including data: null). Selection same as 055.';
