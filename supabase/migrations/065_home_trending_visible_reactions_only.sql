-- 065: get_home_discovery_feed — trending shelf requires visible reactions
--
-- Problem: play sessions alone qualified a project for 「直近7日で反応が集まった作品」.
-- Cards show lifetime feedback / watch counts, so play-only games appeared as
-- 「フィードバック 0 / フォロー 0」 and could sit on page 1 while higher-FB
-- titles were soft-excluded as hero or pushed to page 2.
--
-- Change (trending only):
--   Eligibility: feedback_users_7d + watchers_7d > 0
--   players_7d remains a ranking tie-break among eligible projects (not alone).
-- Newest / updated / hero soft-exclusion / feed_limit / no padding unchanged.
-- Thumbnail http(s)-only rule from 059 unchanged.
...
COMMENT ON FUNCTION public.get_home_discovery_feed() IS
  'Home discovery feed. 065: trending requires feedback_users_7d + watchers_7d > 0 (play alone does not qualify). 059: thumbnail_url http(s) only.';
