-- 054: fix get_home_discovery_feed plpgsql rank variable clash
-- Root cause: RETURNS TABLE(... rank integer ...) makes "rank" a PL/pgSQL
-- variable, so CTE column "rank" is ambiguous (42702).
-- Fix: use internal column name "rn"; map to output "rank" by position only.
...
COMMENT ON FUNCTION public.get_home_discovery_feed() IS
  'Home discovery feed. Internal sort key column is rn to avoid PL/pgSQL '
  'clash with RETURNS TABLE rank (054).';
