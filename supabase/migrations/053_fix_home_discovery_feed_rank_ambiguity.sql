...
BEGIN
  ...
  newest_limited AS (
    SELECT * FROM newest_ranked WHERE rank <= v_limit
  ),
  updated_limited AS (
    SELECT * FROM updated_ranked WHERE rank <= v_limit
  ),
  trending_limited AS (
    SELECT * FROM trending_ranked WHERE rank <= v_limit
  ),
  candidate_ids AS (
    SELECT ARRAY(
  ...
  ORDER BY feed.section, feed.rank;
END;
$$;
