...
- **Studio ホーム UI（v0寄せ）** — `/studio` メインコンテンツを v0 モックに近づけた。ページ見出し「Studio ホーム」、カードはアイコン→グラフ→凡例→フッター（左内訳・右に合計/率の大数字）、気になる動きは横並びグラスカード、下部はクイックアクセス（2×2タイル）と開発のヒント（リスト）を横並び。数値は API のみ。shell / DB / RPC / hooks は変更なし
