...
- **対象** — 基本情報 / ジャンル・タグ / 作品紹介 / サムネイル / プレイ情報 / 公開先・関連リンク / フィードバック問いモーダル
- **関連リンク** — Steam・itch.io の新規追加導線を「追加の掲載先」として復帰。主な遊び先はプレイ情報側、補足リンクは公開先・関連リンク側と整理
- **見出し** — 「公開先・公開設定」→「公開先・関連リンク」。短い補足「主な遊び先は「プレイ情報」に設定します。」のみ
- **DB** — BOOTH / note / App Store 等の新列は migration なしのため未追加（既存列のみ）
- **対象ブランチ** — Preview のみ（main / 本番未変更）

---

## 2026-07-08 — Studioパネル追加の文言整理（入力寄り・サムネ必須・問い上限5）

- **右パネル一覧** — 「未設定」系の現状値列挙を削除。カードは見出し＋必須/任意＋編集ボタン中心
- **「作品を投稿する」** — 右パネル内の重複見出しを削除（Studioパネル見出しは維持）
- **サムネイル必須** — 新規投稿で必須化。AI仮サムネ Coming Soon / 最大枚数の常時説明を削除。既存作品の編集は必須強制しない
- **開発フェーズ** — 選択肢ごとの短い説明を復元（セクション冒頭の長文説明は置かない）
- **正式版公開済み** — 折りたたみ補足削除。確認モーダルを結論＋リスク箇条書き（Special Thanks含む）に整理
- **問い上限** — 版あたり最大 10 → **5**（UI・定数・バリデーション・原典/画面定義ドキュメント）
- **料金・公開形態** — `other` 表示を「その他」に変更（DB値不変）
- **URLラベル** — ブラウザ「ゲームプレイURL」/ ダウンロード「ダウンロードURL」/ 外部「ストア・外部ページURL」
- **関連リンク** — メイン遊び先はプレイ情報側。ストア・配布先カテゴリの新規追加をやめ、既存Steam/itchのみ「追加の販売・配布ページ」として表示。空状態「まだリンクはありません」を削除
- **対象** — Preview のみ（DB migration / main / 本番未変更）
