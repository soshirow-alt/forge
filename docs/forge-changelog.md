...
- **Studio ホーム UI（Preview OK）** — `preview/landing-01` でオーナー Preview 確認済み。実データ3カード・期間切替・X軸/7月棒・フッター揃え・クイックアクセス/開発ヒントを許容範囲と判断。UI微調整は一旦完了。migration **037**（日/週/月粒度）は **Dashboard 適用済み**。production deploy / main merge は別途 GO 判断
