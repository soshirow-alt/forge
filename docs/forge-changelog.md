...
- **修正** — `get_public_feedback_cards` の limit-1 probe + 軽量並列取得。anon が読めない feedback テーブル直読み prefilter は使わない（RLS）。表示仕様（FB 4件・Game/dev-tool hero・Overview IA 等）は維持。切替 Link は即「移動中…」反馈
...
- **掲載** — 投稿と公開を分けない。5カテゴリから選び、作品情報を書いて公開（ゲーム / 音楽・音声 / アセット / 開発ツール / サービス）
- **受け取る / 更新する / つながる** — ゲーム専用のプレイヤー・プレイ可能ver前提をやめる。新ver公開は必須ではない。正式版公開は最終ステップにしない（「正式版になったら公開状態も更新できます」）
- **つながり** — メッセージ / 利用・コラボ相談 / 使用関係。Studio でも閲覧・返信可。Player `/messages` と同じやり取り。マイコミュニティは出さない
- **開発ヒント** — 声 / 開発者 / ゲーム専用語 / 「版」を表示文言から外す。anchor id は維持
- **非実施** — Production / main / 05 announcement / DB write なし

## 2026-08-12 — はじめてガイドを現行Forge仕様へ再設計（Preview）

- **`/guide`** — 初めての人が理解する順に再構成。Forgeとは / 見つける / 試してフィードバック / 掲載 / つながる / フィードバックの次 / PlayerとStudio
- **5カテゴリ** — ゲーム / 音楽・音声 / アセット / 開発ツール / サービス。ゲーム専用説明と「開発者」表記をやめる。一般ユーザーを脇役にしない
- **フィードバック** — 本番はログイン必須（ゲスト送信は Production で無効）。旧「声」は使わない
- **掲載** — Studio 新規投稿 → 5カテゴリ選択 → 専用フォーム → 公開・更新
- **つながり** — メッセージ / 利用・コラボ相談 / 使用関係。承認はライセンス・譲渡・報酬契約そのものではない
- **返礼** — 公開作品がある登録ユーザーが他作品へFBすると、相手に次の発見案内が届くことがある。ポイントや強制交換ではない
- **Player / Studio メッセージ** — 同じやり取り。マイコミュニティは出さない
- **非実施** — Production / main / 05 announcement / DB write なし
...
- **Game Home** — 中央＋左右peekカルーセルを廃止。全体Homeと同じ左1+右3
- **category Home** — game / dev-tool が同じ CategoryHomeHero。1件なら左1+右3プレースホルダ（Production の Unity Font Tool）
- **プレースホルダ** — UI埋め。回転・dot・クリック対象外
- **非実施** — Production / main / 05 announcement / DB write なし

## 2026-08-12 — 作品詳細 Overview: 公開・配布を情報表示に（Preview）

- **遊び方** — ゲーム Overview の基本情報から削除（内部の browser/download/store データ・検索・投稿編集・CTA は維持）
- **CTA** — 強いアクションは上部 primary のみ。Overview 下部で同じ動詞を大きなボタンとして繰り返さない
- **公開・配布** — 旧「公開先」。ダウンロードでもカードは残す。Steam / itch.io / Unity Play / BOOTH / GitHub 等は既存データから表示。不明時は 直接ダウンロード / ブラウザ版 / 外部ストア / **外部ページ**（Owner A: 平台不明の外部URL。ホスト名推測ではない）。ホスト名（supabase / GCS 等）と「その他」は出さない
- **関連リンク** — 「その他」を出さない。X / GitHub / YouTube / 公式サイト等の識別、または「関連ページ」
- **想定時間** — デスクトップ詳細カードは 3+2（5分未満 / 5〜15分 / 15〜30分 ＋ 30分〜1時間 / 1時間以上）
- **グリッド** — ROW1 作品紹介 | カテゴリ基本情報、ROW2 最近の動き | 公開・配布+関連リンク。同一行は天地揃え
- **非実施** — Production / main / 05 announcement / DB / 新規入力欄なし
