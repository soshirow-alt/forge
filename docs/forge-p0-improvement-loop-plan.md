...
**版**: 2026-06-15  
**ステータス**: **GO**（オーナー承認済み）  
**目的**: 価値仮説「感想収集」ではなく **声 → 変化 → 再プレイ** を、開発者が実感できる最小体験で検証する。
...
| # | 仮説 | P0 で測ること |
|---|------|----------------|
| H1 | 開発者は「感想」ではなく **次に直すこと** を得られる | 7日以内に直す項目を1件以上言語化 / Studio 上で上位課題を採用 |
| H2 | 直した後、プレイヤーが **戻ってくる** | 版公開後、当該版の再プレイ人数（または再訪人数）が 1 以上 |

Forge のコアループ（開発者側）:

```
声を受け取る → 次に直すことを決める → 改善・版公開・Devlog → 再プレイを見る
```
...
| KPI | 目標（仮） |
|-----|------------|
| 課題言語化率 | 参加開発者の 2/3 以上が 7日以内に「次に直すこと」を1件以上言える |
| 再プレイ発生率 | 版公開後、少なくとも1作品で再プレイ人数 ≥1 |
| ループ完走 | 3人中1人以上が 1サイクル完走 |
